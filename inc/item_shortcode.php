...
function drstk_item($atts)
{
    ...
    if ($repo == "drs") {
        ...
        // die();
        $data = $data->_source;
        $thumbnails = $data->fields_thumbnail_list_tesim;
        $thumbnail = $thumbnails[$num];
        $masterThumbnail = $thumbnails[4];
        $thumbnailLink = "https://repository.library.northeastern.edu" . $thumbnail;
        $masterThumbnailLink = "https://repository.library.northeastern.edu";
        
        // convoluted structure so I don't have refactor everything in 2 days
        $thumbnail = $thumbnailLink;
        $master = $masterThumbnailLink;
