...
export function registerTimelineBlock() {
	registerBlockType("drs-tk/timeline", {
		...
		edit: (props) => {
			...
			const addItem = () => {
				const block = createBlock("drs-tk/timeline-item");
				dispatch("core/block-editor").insertBlock(block, undefined, clientId);
			};

			const toggleAlternate = () =>
				setAttributes({ isAlternating: !isAlternating });

			const classes = classnames("wp-block-drs-tk-timeline", {
			...
			return (
				<>
					...
					<ul className={classes}>
						<InnerBlocks
							allowedBlocks={["drs-tk/timeline-item"]}
							template={[["drs-tk/timeline-item"]]}
