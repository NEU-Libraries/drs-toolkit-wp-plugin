jQuery(document).ready(function ($) {
    ...
    if (q && q != '') {
        ...
        $('#drs-selection .col-md-10').append(
            "<a class='themebutton btn btn-more' href='#' data-type='q' data-val='" + params.q + "'>" + params.q + " <span class='fa fa-close'></span></a>"
        );
    }
    ...
    function getData(params) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                const drsContent = $('#drs-content');
                const errorNull = data == null || data == -1;
                // if data is null
                if (errorNull) {
                    drsContent.html(errors.search.fail_null);
                    return;
                }
                // if data is an error
                if (data.error) {
                    drsContent.html(template == 'collections' ? errors.search.no_sub_collections : errors.search.no_results);
                    return;
                }
                // if data is empty
                if (data.response.response.numFound == 0) {
                    drsContent.html(errors.search.no_results);
                }

                paginate(data.pagination.table); //send to paginate function

                if (params.show_facets) {
    ...
    function paginate(data) {
        ...
        $('#drs-per-page-div').html(
            "<h6>Show <select id='drs-per-page'><option val='10'>10</option><option val='20'>20</option><option val='50'>50</option></select> per page</h6>"
        );
    ...
    function facetize(data) {
        ...
        $('#drs-facets').before(
            "<button class='themebutton button btn visible-phone hidden-tablet hidden-desktop drs-facet-toggle hidden-md hidden-lg visible-sm visible-xs'>Show Facets</button>"
        );
    } //end facetize
            ...
            if (Object.keys(data.facet_fields[facet]).length > 0) {
                ...
                $.each(facet_array, function (index, val_q) {
                    ...
                    if (this_facet_count != undefined) {
                        this_facet =
                            "<a href='#' class='drs-facet-val row'><div class='three_fourth col-xs-8'>" +
                            this_facet_name +
                            "</div><div class='one_fourth col-xs-4 last'>" +
                            this_facet_count +
                            '</div></a>';
                        if (i <= 5) {
    ...
    function resultize(data) {
        ...
        $.each(data.docs, function (doc, doc_vals) {
            ...
            if (klass == 'CoreFile') {
                klass = getShortName(doc_vals.canonical_class_tesim);
            }
    ...
    function clickable(data) {
        ...
        $('#drs-facets a').on('click', function (e) {
            ...
            $('#drs-selection').show();
            $('#drs-selection .col-md-10').append(
                "<a class='themebutton btn btn-more' href='#' data-type='f' data-facet='" +
                    facet +
                    "' data-val='" +
                    facet_val +
                    "'>" +
            ...
            $([document.documentElement, document.body]).animate({ scrollTop: $('#title-container').offset().top }, 1500);
    ...
    $('#drs-sort').html(
        "<h6>Sort By: <select id='drs-sort-option'><option value='score+desc%2C+system_create_dtsi+desc'>Relevance</option><option value='title_ssi%20asc'>Title A-Z</option><option value='title_ssi%20desc'>Title Z-A</option><option value='creator_ssi%20asc'>Creator A-Z</option><option value='creator_ssi%20desc'>Creator Z-A</option><option value='system_modified_dtsi%20asc'>Date (earliest to latest)</option><option value='system_modified_dtsi%20desc'>Date (latest to earliest)</option></select></h6>"
    );
    ...
    function titleize(str) {
        if (facets_to_display[str]) return facets_to_display[str];
        if (niec_facets[str]) return niec_facets[str];

        str = str.replace(/_tesim|_sim|_ssim|_/g, ' ').replace(/\w\S*/g, (txt) => txt.charAt(0).toUpperCase() + txt.substr(1).toLowerCase());
        return str;
    }
    ...
    function getWPData(query, page = 1) {
        if (template == 'search') {
            $.ajax({
                type: 'GET',
                url: browse_obj.ajax_url,
                ...
                success: function (data) {
                    $('#secondary').html(
                        "<div class='panel panel-default'><div class='panel-heading'><b>" + related_content_title + "</b></div><div class='panel-body'>" + data + '</div></div>'
                    );
    ...
    function fixWPPagination() {
        $('#secondary .pagination a').on('click', function (e) {
            ...
            getWPData(params.q, wp_page);
        });
    }

    function getShortName(klass) {
        const mapping = {
            AudioFile: 'Audio',
            ImageLargeFile: 'Image',
            ImageMasterFile: 'Image',
            ImageMediumFile: 'Image',
            ImageSmallFile: 'Image',
            ImageThumbnailFile: 'Image',
            MsexcelFile: 'Dataset',
            MspowerpointFile: 'Presentation',
            MswordFile: 'Document',
            PdfFile: 'Document',
            TextFile: 'Document',
            VideoFile: 'Video',
