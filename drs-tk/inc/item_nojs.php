...
$url = "http://repository.library.northeastern.edu/api/v1/files/" . $item_pid;
...
function get_item_breadcrumbs(){
  ...
  $breadcrumb_url = "http://repository.library.northeastern.edu/api/v1/search/".$collection."?"."f['id'][]=".$item_pid;
...
function parse_this_breadcrumb($doc){
  ...
  if ($doc->fields_parent_id_tesim){
    ...
    if ($parent != $collection){
      $breadcrumb_url = "http://repository.library.northeastern.edu/api/v1/search/".$collection."?"."f['id'][]=".$parent;
      get_this_breadcrumb($breadcrumb_url);
    } else {
